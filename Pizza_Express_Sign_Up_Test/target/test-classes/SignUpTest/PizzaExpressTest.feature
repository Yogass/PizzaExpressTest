Feature: Account Registration Test
  ...
  Scenario: User registers on the website
      ...
      And User writes "Alex" in First name input field
      And User writes "Jan" in Last name input field
      And User writes "aleksandra.jankowiak@yahoo.com" in Email address input field
      ...
      And User writes "PizzaTest2321" in Create password inout field
      ...
      Then User should see ALEX'S PIZZAEXPRESS sign

  Scenario: User check email for confirmation email
    Given Yahoo website is open
    When User writes "aleksandra.jankowiak" into the email input field
    And User clicks Next button
    Then Password request page should be open
    Then User writes "Strzegom1" into password input field and clicks Next button
